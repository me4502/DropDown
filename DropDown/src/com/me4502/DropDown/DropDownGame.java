...
public class DropDownGame implements ApplicationListener {
	...
	public void render() {
		...
		if((lev.lastCameraMove > 5 || lev.startedReading) && lev.readingLine <= lev.base.getDialogueLines().size()-1) {
			...
			sBack.setBounds(0-Gdx.graphics.getWidth()/2, 0-Gdx.graphics.getHeight()/2, Gdx.graphics.getWidth(), Gdx.graphics.getHeight()/3);
			sBack.draw(batch);
			Sprite ash = new Sprite(ashleyDialogAnimation.getKeyFrame(stateTime, true));
			ash.setOrigin(0f, 0f);
			ash.setBounds(0-Gdx.graphics.getWidth()/2 + 32, 0-Gdx.graphics.getHeight()/2 + 32, 128f, 128f);
			ash.draw(batch);
			mainFont.setColor(0f, 0f, 0f, 1f);
			List<String> lines = new LinkedList<String>();
			for(String string : lev.base.getDialogueLines().get(lev.readingLine).split("\\|"))
				lines.add(string);
			int ind = 0;
			for(String string : lines)
				mainFont.draw(batch, string, 0-Gdx.graphics.getWidth()/2 + 32+128+32, 0-164 + (isAndroid() ? 48 : 0) - (mainFont.getBounds(string).height+4)*ind++);

			smallFont.setColor(0f,0f,0f,1f);
			String pressString = isAndroid() ? "Tap " : "Press E ";
			smallFont.draw(batch, "(" + pressString + "to continue)", 0-smallFont.getBounds("(" + pressString + "to continue)").width/2, 0-Gdx.graphics.getHeight()/2 + smallFont.getCapHeight() + 16);
			smallFont.draw(batch, "Ashley", 0-Gdx.graphics.getWidth()/2+30+ash.getWidth()/2-smallFont.getBounds("Ashley").width/2, 0-Gdx.graphics.getHeight()/2 + smallFont.getCapHeight() + 16);
		...
		if(lev.isGameOver) {
			TextBounds b = largeFont.getBounds("Game Over");
			largeFont.setColor(1f, 0f, 0f, 1f);
			largeFont.draw(batch, "Game Over", 0-b.width/2, 0-b.height/2);
			TextBounds b2 = largeFont.getBounds("Click to Play Again!");
			largeFont.draw(batch, "Click to Play Again!", 0-b2.width/2, 0-64-b2.height/2);
		} else if(isPaused) {
			TextBounds b = largeFont.getBounds("Paused");
			largeFont.setColor(1f, 1f, 1f, 1f);
			largeFont.draw(batch, "Paused", 0-b.width/2, 0-b.height/2);
			TextBounds b2 = largeFont.getBounds("Press Escape To Resume!");
			largeFont.draw(batch, "Press Escape To Resume!", 0-b2.width/2, 0-64-b2.height/2);
		}
