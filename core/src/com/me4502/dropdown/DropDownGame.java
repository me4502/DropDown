...
public class DropDownGame implements ApplicationListener {
	...
	public void create() {
		...
		SoundStorage.getSound("background").loop();
	}
	...
	public void render() {
		...
		for(Tile[] tiles : lev.base.getTiles()) {
			for (Tile tile : tiles) {
				if (tile == null || tile.getType() != Materials.TORCH) {
					continue;
				}
				batch.draw(TextureStorage.getTexture("light"),
						tile.getX() * 64 * lev.getBaseYScale() - Gdx.graphics.getWidth() / 2 - lev.cameraXOffset - lightSize / 2 + 16,
						tile.getY() * 64 * lev.getBaseYScale() - Gdx.graphics.getHeight() / 2 - lightSize / 2 + 16, lightSize, lightSize);
			}
		}
		...
		if((lev.lastCameraMove > 5 || lev.startedReading) && lev.readingLine <= lev.base.getDialogueLines().size()-1) {
			...
			for(String string : lines) {
				glyphLayout.setText(mainFont, string);
				mainFont.draw(batch, glyphLayout, -Gdx.graphics.getWidth() / 2 + 32 + 128 + 32,
						-164 - (glyphLayout.height + 4) * ind++);
			}

			smallFont.setColor(0f,0f,0f,1f);

			String pressString = "Press E ";
			glyphLayout.setText(smallFont, "(" + pressString + "to continue)");
			smallFont.draw(batch, glyphLayout, 0-glyphLayout.width/2,
					-Gdx.graphics
					.getHeight() / 2 + smallFont.getCapHeight() + 16);

			glyphLayout.setText(smallFont, "Ashley");
			smallFont.draw(batch, glyphLayout, -Gdx.graphics.getWidth() / 2 +30+ash.getWidth()/2-glyphLayout.width/2, -Gdx.graphics
					.getHeight() / 2 + smallFont.getCapHeight() + 16);
			if(Gdx.input.isKeyPressed(Keys.E) && clickTimer <= 0) {
				lev.readingLine += 1;
				clickTimer = 20;
			}
			clickTimer--;
	...
	int clickTimer = 0;
	Animation<TextureRegion> ashleyDialogAnimation;
	float stateTime;
